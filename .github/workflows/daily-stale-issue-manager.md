...
description: |
  ...
  - Only considers issues older than 0 days (0 days) to minimize API usage.
...
on:
  ...
  #schedule: daily
  reaction: "eyes"
  # TODO: restore fork guard after testing
  # steps:
...
**Stale threshold**: 0 days (approximately 0 days).
...
1. Search for open issues with that label created more than 0 days ago.
   Use the GitHub search query:
   ```
   repo:<owner>/<repo> is:issue is:open label:"<kind-label>" created:<YYYY-MM-DD
   ```
   where the date is today minus 0 days. This keeps the result set small by excluding issues
...
> activity in over 0 days. It will be closed if no further activity occurs within the next
> run. If this is still relevant, please leave a comment explaining why it should remain open.
...
#### 3c. Close issues that have been stale for at least 0 days
...
3. Has at least 0 days passed since the label was added?

If the label has been present for 0 or more days with no meaningful activity since it was applied,
close the issue, add the `bot/stale-issue-manager/closed` label, and post this comment:

> We're closing this issue because it hasn't been active nor has been prioritized. If this is still needed in the latest version of Rancher, please re-open and let us
